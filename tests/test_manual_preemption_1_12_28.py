...
def test_current_version_and_existing_priority_are_preserved() -> None:
    assert 'VERSION = "1.12.43"' in SERVER
    assert 'CONNECTOR_VERSION = "1.12.43"' in CONNECTOR
    assert 'ENGINE_VERSION = "1.12.43"' in TELEMETRY
...
def test_no_new_physical_retry_was_added() -> None:
    ...
    # A mudança de 1.12.43 é de scheduling/telemetria, não de repetição do comando.
