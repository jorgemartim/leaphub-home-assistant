...
def load(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv('LEAPHUB_OCPP_QUEUE_MAX', '100')
...
def test_parallel_reconnect_writes_do_not_lock(tmp_path, monkeypatch):
    m=load(tmp_path, monkeypatch); m.TARGETS_BY_NAME={'staging':m.ApiTarget('staging','https://example.invalid/ocpp','x'*32)}
    errors=[]
    def worker(i):
        try:
            for n in range(20):
...
def test_reconnect_diagnostics_are_aggregate(tmp_path, monkeypatch):
