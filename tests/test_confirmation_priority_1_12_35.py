...
checks = {
    "version": 'VERSION = "1.12.39"' in server and 'ENGINE_VERSION = "1.12.39"' in telemetry,
    ...
    "pipeline_frozen": "Validate, build and publish amd64 image" in build and "matrix:" not in build and "pytest" not in build,
}
failed = [name for name, ok in checks.items() if not ok]
if failed:
    raise SystemExit(f"failed: {failed}")
print({"ok": True, "checks": len(checks), "version": "1.12.39"})
