...
checks = {
    ...
    "version": any(v in (ROOT / "leaphub_gateway" / "config.yaml").read_text(encoding="utf-8") for v in ('version: "1.12.48"', 'version: "1.12.50"')),
}
failed = [name for name, ok in checks.items() if not ok]
if failed:
    raise SystemExit("pre-dispatch token recovery contract failed: " + ", ".join(failed))
print({"ok": True, "checks": len(checks), "version": "1.12.50"})
