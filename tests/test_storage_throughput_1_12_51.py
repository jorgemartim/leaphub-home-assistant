...
def test_maintenance_throttle_never_delays_session_expiry():
    ...
    Regressão de 1.12.51: a primeira versão do throttle saía do método antes da
...
def test_image_selftest_accepts_both_journal_modes():
    """O autoteste da imagem não pode voltar a prender um journal fixo.

    A 1.12.50 reprovou no build justamente por isso: o volume da imagem aceitou
    WAL e o Dockerfile exigia DELETE.
    """
    dockerfile = (APP / "Dockerfile").read_text(encoding="utf-8")
    assert "assert journal in ('wal', 'delete'), journal" in dockerfile
    assert "assert engine.storage_journal_mode == journal" in dockerfile
    assert "assert journal == 'delete', journal" not in dockerfile
    assert "engine.close_storage()" in dockerfile


def test_delivery_reuses_one_connection_across_batches():
    """A conexão TLS é aberta uma vez e reaproveitada pelo lote seguinte."""
    with tempfile.TemporaryDirectory(prefix="leaphub-delivery-reuse-") as tmp:
        engine = new_engine(Path(tmp))
        try:
            opened = {"count": 0}
            sent = []
            ...
            class _Connection:
                ...
                def request(self, method, target, body=None, headers=None):
                    sent.append((method, target, dict(headers or {})))
            ...
            telemetry.http.client.HTTPSConnection = _Connection
            try:
                url = "https://example.invalid/leap/api/internal/telemetry/events"
                first = engine._post_delivery(url, {"X-Test": "1"}, b"{}")
                second = engine._post_delivery(url, {"X-Test": "2"}, b"{}")
            finally:
            ...
            # Dois lotes, um único handshake.
            assert opened["count"] == 1
            assert len(sent) == 2
            assert all(h.get("Connection") == "keep-alive" for _m, _t, h in sent)
            assert all(h.get("Content-Length") == "2" for _m, _t, h in sent)
        finally:
            close_engine(engine)


def test_delivery_connection_is_dropped_on_transport_error():
    """Socket meio-fechado nunca volta ao pool."""
    with tempfile.TemporaryDirectory(prefix="leaphub-delivery-drop-") as tmp:
        engine = new_engine(Path(tmp))
        try:
            class _Broken:
                def __init__(self, *_args, **_kwargs) -> None:
                ...
                def getresponse(self):  # pragma: no cover - nunca alcançado
                    raise AssertionError("não deveria ler resposta")

                def close(self) -> None:
                    self.closed = True

            original = telemetry.http.client.HTTPSConnection
            telemetry.http.client.HTTPSConnection = _Broken
            try:
                raised = False
                try:
                    engine._post_delivery("https://example.invalid/x", {}, b"{}")
                except OSError:
                    raised = True
                assert raised
...
def test_collection_status_reports_saturation():
    with tempfile.TemporaryDirectory(prefix="leaphub-collection-status-") as tmp:
        engine = new_engine(Path(tmp), {"telemetry_poll_workers": 2})
        try:
            status = engine.collection_status()
            assert status["poll_workers"] == 2
            assert status["polls_in_flight"] == 0
            assert status["workers_saturated"] is False
            assert status["journal_mode"] in {"wal", "delete"}

            engine._inflight.update({"a", "b"})
            saturated = engine.collection_status()
            assert saturated["polls_in_flight"] == 2
            assert saturated["workers_saturated"] is True
            engine._inflight.clear()

            assert engine.status_fast()["collection"]["poll_workers"] == 2
        finally:
            close_engine(engine)
