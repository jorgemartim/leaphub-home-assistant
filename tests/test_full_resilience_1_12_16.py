...
with tempfile.TemporaryDirectory(prefix="leaphub-1-12-16-") as tmp:
    ...
    check('version: "1.12.18.2"' in (APP / "config.yaml").read_text(), "Versão do App divergente")

if failures:
    raise SystemExit("full resilience 1.12.18.2 failed:\n- " + "\n- ".join(failures))
print({"ok": True, "checks": 12, "version": "1.12.18.2"})
