...
assert target == "1.12.53"
assert config["version"] in {"1.12.48", "1.12.53"}
assert config["image"] == "ghcr.io/jorgemartim/leaphub-gateway"
assert re.findall(r"^##\s+(.+)$", (APP / "CHANGELOG.md").read_text(encoding="utf-8"), re.M) == ["1.12.53"]
...
assert (APP / "RELEASE-1.12.53.md").is_file()
print({"ok": True, "version": "1.12.53", "distribution": "prebuilt-staged", "fairness": "owner_user"})
