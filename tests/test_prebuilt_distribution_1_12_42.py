...
assert target == '1.12.42'
assert config['version'] in {'1.12.41', '1.12.42'}
...
assert (APP / 'RELEASE-1.12.42.md').is_file()
assert re.findall(r'^##\s+(.+)$', (APP / 'CHANGELOG.md').read_text(encoding='utf-8'), re.M) == ['1.12.42']
print({'ok': True, 'version': '1.12.42', 'distribution': 'prebuilt-staged'})
