...
checks = {
    'version': CONFIG.get('version') == '1.12.41',
    ...
    'pytest_only_for_validation': '"pytest>=8,<10"' in BUILD and 'pip install --requirement' not in BUILD,
    ...
    'single_changelog_heading': re.findall(r'^##\s+(.+)$', CHANGELOG, flags=re.MULTILINE) == ['1.12.41'],
}
failed=[name for name, ok in checks.items() if not ok]
if failed:
    raise SystemExit(f'failed: {failed}')
print({'ok': True, 'checks': len(checks), 'version': '1.12.41'})
