# Recuperação GitHub — Gateway 1.12.67

1. Envie somente os arquivos de `CHANGED-FILES-1.12.67.txt`.
2. Confirme `leaphub_gateway/RELEASE_TARGET` em `1.12.67`.
3. Preserve `leaphub_gateway/config.yaml` em `1.12.66` até a imagem nova estar
   pública. O commit da 1.12.67 não toca esse arquivo, então a promoção
