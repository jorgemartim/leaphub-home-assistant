# Recuperação GitHub — Gateway 1.12.68

1. Envie somente os arquivos de `CHANGED-FILES-1.12.68.txt`.
2. Confirme `leaphub_gateway/RELEASE_TARGET` em `1.12.68`.
3. Preserve `leaphub_gateway/config.yaml` em `1.12.67` até a imagem nova estar
   pública. O commit da 1.12.68 não toca esse arquivo, então a promoção
